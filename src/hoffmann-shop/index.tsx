...
type hoffmannCartWidgetState = {
  state?: "checkout" | null;
  cartItems?: CartItem[];
  selectedCartItemId?: string | null;
};

type hoffmannCartWidgetProps = {
  cartItems?: CartItem[];
  widgetState?: Partial<hoffmannCartWidgetState> | null;
};

const SERVICE_FEE = 3;
const DELIVERY_FEE = 2.99;
const TAX_FEE = 3.4;
const CONTINUE_TO_PAYMENT_EVENT = "hoffmann-shop:continue-to-payment";
...
const createDefaultWidgetState = (): hoffmannCartWidgetState => ({
  state: null,
...
function App() {
  ...
  const widgetProps = useWidgetProps<hoffmannCartWidgetProps>(() => ({}));
  const [widgetState, setWidgetState] = useWidgetState<hoffmannCartWidgetState>(
  ...
  const serverData = useMemo(() => {
    ...
    try {
      const json = typeof toolOutput === 'string' ? JSON.parse(toolOutput) : toolOutput.result ? JSON.parse(toolOutput.result) : toolOutput;
      return json;
    } catch (e) { return null; }
  }, [toolOutput]);

  const defaultCartItems = useMemo(() => {
    if (serverData?.cartItems && Array.isArray(serverData.cartItems)) {
      return serverData.cartItems;
    }
  ...
  const updateWidgetState = useCallback(
    (partial: Partial<hoffmannCartWidgetState>) => {
...
createRoot(document.getElementById("hoffmann-shop-root")!).render(
  <BrowserRouter>
    <App />
  </BrowserRouter>
);
